...
public class BaseSerializer implements ISerializer<Object> {
    ...
    public void deserializeTo(Object serialized, Class<?> targetClass, EnhancedConfig config, Object to) {
        ...
        Arrays.stream(targetClass.getDeclaredFields())
                .forEach(field -> {
                    ...
                    Type[] typeArgs = generic instanceof ParameterizedType ? ((ParameterizedType) generic).getActualTypeArguments() : null;
    ...
    class SerializationHandler {
        private Object handleObject(Object configValue, Serializer serializerAnn, EnhancedConfig config) {
            ...
            ISerializer<?> serializer = ConfigSerializerRegistry.getInstance().getSerializer(configValue.getClass());
            if(serializerAnn != null) serializer = TryCatchUtil.tryAndReturn(() -> serializerAnn.value().getDeclaredConstructor().newInstance());
            if(isEnclosedInConfig(configValue.getClass())) serializer = ConfigSerializerRegistry.CustomSerializers.BASE;

            return serializer == null ? configValue : useSerializer(serializer,configValue,config);
        }
    ...
    class DeserializationHandler {
        private Object handleObject(Object configValue, Class<?> clazz, Type[] typeArgs, Serializer serializerAnn, EnhancedConfig config) {
            ...
            if(Map.class.isAssignableFrom(clazz)) return handleMapDeserialization(
                    configValue instanceof ConfigurationSection ? ((ConfigurationSection) configValue).getValues(false) : (Map<?,?>) configValue
                    , typeArgs[1],serializerAnn, config);

            ISerializer<?> serializer = ConfigSerializerRegistry.getInstance().getSerializer(clazz);
            if(serializerAnn != null) serializer = TryCatchUtil.tryAndReturn(() -> serializerAnn.value().getDeclaredConstructor().newInstance());
            if(isEnclosedInConfig(clazz)) serializer = ConfigSerializerRegistry.CustomSerializers.BASE;

            return serializer == null ? configValue : useDeserializer(serializer,configValue,clazz,config);
        }

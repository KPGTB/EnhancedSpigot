...
public class EnhancedRequest {
	...
	public EnhancedResponse send() throws IOException {
		...
		TryCatchUtil.tryRun(() -> connection.setRequestMethod(this.method.name()), (e) -> {
		...
		if (!this.cookies.isEmpty()) {
			...
			this.cookies.forEach(cookie -> cookieManager.getCookieStore()
														.add(null, cookie));
			this.headers.putIfAbsent("Cookie", cookieManager.getCookieStore()
															.getCookies()
															.stream()
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(status > 299 ?

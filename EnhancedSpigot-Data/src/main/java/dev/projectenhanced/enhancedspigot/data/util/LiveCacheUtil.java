...
import dev.projectenhanced.enhancedspigot.data.cache.LiveCache;
...
public class LiveCacheUtil {
	private static boolean isLiveCache(IAsyncSavableCache<?, ?> cache) {
		return cache instanceof LiveCache<?, ?>;
	}
	...
	public static <T> void createForeign(IAsyncSavableCache<?, ?> cache, T foreign, ForeignCollection<T> collection) {
		if (cache instanceof LiveCache<?, ?>) {
			LiveCache<?, ?> lc = (LiveCache<?, ?>) cache;
			lc.runAsync(
				() -> TryCatchUtil.tryRun(() -> collection.add(foreign)), lc.getAsyncLivePriorityMap()
		...
		if (cache instanceof LiveCache<?, ?>) {
			...
			lc.runAsync(
				() -> TryCatchUtil.tryRun(() -> collection.getDao()
					.delete(foreign)), lc.getAsyncLivePriorityMap()
					.getLiveActionPriority(), System.currentTimeMillis()
			);
		}
	}
	...
	public static void clearForeign(IAsyncSavableCache<?, ?> cache, ForeignCollection<?> collection) {
		if (cache instanceof LiveCache<?, ?>) {
			LiveCache<?, ?> lc = (LiveCache<?, ?>) cache;
			lc.runAsync(
				() -> TryCatchUtil.tryRun(collection::clear), lc.getAsyncLivePriorityMap()
					.getLiveActionPriority(), System.currentTimeMillis()
			);
		}
	}

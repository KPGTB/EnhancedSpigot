...
public class CollectionUtil {
	...
	public static Type[] getTypesFromString(String[] elements, int idx) throws ClassNotFoundException {
		...
		if (isMap) {
			return new Type[]{Class.forName(mapElements[0]), TypeToken.getParameterized(Class.forName(mapElements[1]), getTypesFromString(elements, ++idx)).getType()};
		}

		return new Type[]{TypeToken.getParameterized(Class.forName(elements[idx]), getTypesFromString(elements, ++idx)).getType()};
	}
}